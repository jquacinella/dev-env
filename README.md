...
To check if newer versions of tools are available, run the updater script:
...
To apply the available updates directly to `vars/versions.yml`:

```bash
python3 scripts/update-versions.py --write
```

The script preserves each pin's existing format (e.g. a leading `v`, or the
`gping-v` prefix), so only the version numbers change. You can also limit the
check to specific tools:

```bash
# Check a single tool (short name or the full variable name both work)
python3 scripts/update-versions.py --only gitui

# Check a few at once
python3 scripts/update-versions.py --only fzf,bat,ripgrep
```

**Rate limits**: unauthenticated GitHub API calls are limited to 60/hour. A
full run uses ~40 calls. Set `GITHUB_TOKEN` (or `GH_TOKEN`) to raise the limit
to 5000/hour:

```bash
GITHUB_TOKEN=ghp_xxx python3 scripts/update-versions.py
```

**Color output**: install Rich (`pip install rich`) for colorized output —
the "updates available" section is highlighted in orange and errors in red.
Without Rich the script prints plain text.

**Coverage**: all version-pinned CLI tools, ZSH plugins, and the
spaceship-prompt theme are tracked. Two pins are intentionally excluded:
`zsh-fzf-history-search` (tracks the `master` branch) and Podman (installed
from Ubuntu's apt repositories — update it with
...
Updates available (3):
  Tool              Current          Latest
 ──────────────────────────────────────────
  ripgrep_version   15.1.0      ->   15.2.0
  espanso_version   v2.3.0      ->   v2.4.0
  gping_version     gping-v1.20.1 -> gping-v1.21.0
...
2. Update the desired version(s). Either let the script edit `vars/versions.yml`
   automatically:
   ```bash
   python3 scripts/update-versions.py --write
   ```
   or edit `vars/versions.yml` by hand:
   ```yaml
...
6. Add the tool to the `TOOLS` map in `scripts/update-versions.py` so its
   version is tracked (map the `*_version` variable to its GitHub repo or
   crates.io crate)
...
├── scripts/
│   └── update-versions.py # Checks upstream for latest versions, updates versions.yml
├── vars/
